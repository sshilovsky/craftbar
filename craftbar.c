...
void scale_icon(task * tk)
{
	...
	if (tk->mask != None) {
		...
		mgc =
		    XCreateGC(dd, mk, GCGraphicsExposures | GCSubwindowMode,
			      &gcv);
	}
	...
	for (y = ICONHEIGHT - 1; y >= 0; y--) {
		...
		for (x = ICONWIDTH - 1; x >= 0; x--) {
			xx = (x * w) / ICONWIDTH;
			if (d != scr_depth)
				XCopyPlane(dd, tk->icon, pix, fore_gc, xx, yy,
					   1, 1, x, y, 1);
			else
				XCopyArea(dd, tk->icon, pix, fore_gc, xx, yy, 1,
					  1, x, y);
			if (mk != None)
				XCopyArea(dd, tk->mask, mk, mgc, xx, yy, 1, 1,
					  x, y);
		}
	}
...
int is_visible_on_desktop(Window win, int desk)
{
	int client_desk = -1;
	unsigned long *data;
...
int get_current_desktop()
{
	...
	if (wm_use_ewmh)
		data =
		    get_prop_data(root_win, atom__NET_CURRENT_DESKTOP,
				  XA_CARDINAL, 0);
	else
		data =
		    get_prop_data(root_win, atom__WIN_WORKSPACE, XA_CARDINAL,
				  0);
...
void gui_draw_task(taskbar * tb, task * tk)
{
	...
	if (tk->focused) {
		...
		draw_line(tb, x + 2, WINHEIGHT - 2, x + taskw - 2,
			  WINHEIGHT - 2);
...

	{
		...
		len = strlen(tk->name);
		while (XTextWidth(xfs, tk->name, len) >=
		       taskw - (text_x - x) - 2 && len > 0)
			len--;

		if (tk->iconified) {
			/* draw task's name dark (iconified) */
			set_foreground(3);
			XDrawString(dd, tb->win, fore_gc, text_x, text_y + 1,
				    tk->name, len);
			set_foreground(4);
		} else {
			set_foreground(5);
		...
		XDrawString(dd, tb->win, fore_gc, text_x, text_y, tk->name,
			    len);
	}
...
void gui_draw_clock(taskbar * tb)
{
	...
	set_foreground(5);
	XDrawString(dd, tb->win, fore_gc, x + TEXTPAD - 1, text_y, time_str, 5);
}
...
void draw_up_triangle(taskbar * tb)
{
	...
	draw_line(tb, left_arrow_x, (WINHEIGHT - 4) / 2 + 3, left_arrow_x + 3,
		  (WINHEIGHT - 4) / 2);
	draw_line(tb, left_arrow_x + 3, (WINHEIGHT - 4) / 2, left_arrow_x + 6,
		  (WINHEIGHT - 4) / 2 + 3);
	draw_line(tb, left_arrow_x + 1, (WINHEIGHT - 4) / 2 + 3,
		  left_arrow_x + 5, (WINHEIGHT - 4) / 2 + 3);
}

void draw_down_triangle(taskbar * tb)
{
	draw_line(tb, right_arrow_x, (WINHEIGHT - 4) / 2, right_arrow_x + 3,
		  (WINHEIGHT - 4) / 2 + 3);
	draw_line(tb, right_arrow_x, (WINHEIGHT - 4) / 2, right_arrow_x + 4,
		  (WINHEIGHT - 4) / 2 + 2);
	draw_line(tb, right_arrow_x, (WINHEIGHT - 4) / 2, right_arrow_x + 5,
		  (WINHEIGHT - 4) / 2 + 1);
	draw_line(tb, right_arrow_x, (WINHEIGHT - 4) / 2, right_arrow_x + 6,
		  (WINHEIGHT - 4) / 2);
}
...
void taskbar_read_clientlist(taskbar * tb)
{
	...
	if (!win) {
		...
		win =
		    get_prop_data(root_win, atom__WIN_CLIENT_LIST, XA_CARDINAL,
				  &num);
		if (!win) {
			/* failed, let's try another unified one */
			win =
			    get_prop_data(root_win,
					  atom__NET_CLIENT_LIST_STACKING,
					  XA_WINDOW, &num);
			if (!win)
				return;
		}
...
void switch_desk(taskbar * tb, int rel)
{
	...
	if (wm_use_ewmh)
		data =
		    get_prop_data(root_win, atom__NET_NUMBER_OF_DESKTOPS,
				  XA_CARDINAL, 0);
	else
		data =
		    get_prop_data(root_win, atom__WIN_WORKSPACE_COUNT,
				  XA_CARDINAL, 0);
	if (!data)
	...
	xev.format = 32;
	xev.data.l[0] = want;
	XSendEvent(dd, root_win, False, SubstructureNotifyMask,
		   (XEvent *) & xev);
}
